...
//1.0
void SeatingArrangementandReservation();
...
void mainmenu()
{
    ...
    {
        case 1:
            SeatingArrangementandReservation();
            break;
...
/*取消预订模块允许用户输入预订ID来取消所选的机票预订。输入无效（不存在）的预订 ID 时，系统会提示错误消息*/
...
void ReservationCancellationManyTime()
{
    ...
    cout << "\nWhich ticket number do you want to cancel ? \n";
    ...
    else if(cancell == 'n' || cancell== 'N')
    {
		...
		cout << "Your ticket was unsuccessfully canceled ! \n\n";
        mainmenu();
        return;
    }
